...
describe('Parte B: Sistema de Minijuegos en Carreras Clave (B.1 - B.7)', () => {
  ...
  it('B.1 / B.7: respeta la alta probabilidad de aparicion (>=80%) en carreras clave (Prompt 10)', () => {
    let generados = 0;
    const total = 500;
    ...
    expect(porcentaje).toBeGreaterThanOrEqual(0.80);
    expect(PROBABILIDAD_APARICION_MINIJUEGO).toBe(0.85);
  });
  ...
  it('B.2 / B.3 - B.5: exito en opcion optima otorga +45 score y +1 stat sin restar en fallo', () => {
    ...
    if (mgContext) {
      ...
      if (resultado.exito) {
        expect(resultado.bonoModificadorScore).toBe(45);
